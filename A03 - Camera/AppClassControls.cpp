...
void Application::CameraRotation(float a_fSpeed)
{
	...
	vector3 right = glm::normalize(glm::cross(
		m_pCamera->GetAbove() - m_pCamera->GetPosition(),
		m_pCamera->GetTarget() - m_pCamera->GetPosition()
	));

	//Quaternions used to rotate the target with
	quaternion xRotQuat = glm::angleAxis(-fAngleX, right);
	//This one uses the above vector to rotate around
	quaternion yRotQuat = glm::angleAxis(-fAngleY, glm::normalize(m_pCamera->GetAbove() - m_pCamera->GetPosition()));

	//Setting the target to the local target multiplied by the different quaternions
	m_pCamera->SetTarget(((m_pCamera->GetTarget() - m_pCamera->GetPosition()) * xRotQuat * yRotQuat) + m_pCamera->GetPosition());
