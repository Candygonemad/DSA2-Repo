...
//Move Camera forward based on local position
void MyCamera::MoveForward(float a_fDistance)
{
	//Find the forward vector based off of the local forward vector
	vector3 forward = glm::normalize(m_v3Target - m_v3Position) * a_fDistance;
...
//Move Camera vertical based on local position
void MyCamera::MoveVertical(float a_fDistance)
{
	//Find the vertical vector based off of the local above vector
	vector3 vertical = glm::normalize(m_v3Above - m_v3Position)* a_fDistance;

	//Add to go up
	m_v3Position += vertical;
	m_v3Target += vertical;
	m_v3Above += vertical;
}

//Move Camera sideways based on local position
void MyCamera::MoveSideways(float a_fDistance)
{
	//Find the right vector based off of the local above and the local target
	vector3 right = (glm::normalize(glm::cross(m_v3Above - m_v3Position, m_v3Target - m_v3Position))) * a_fDistance;
	
	//Subtract to move right
	m_v3Position -= right;
	m_v3Target -= right;
	m_v3Above -= right;
}